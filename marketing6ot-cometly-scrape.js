...
const EMAIL = 'marketing6ot@gmail.com';
const PASS = '1Apple2345!@';
...
async function selectMarkPreset(page) {
 console.log(' → Selecting Mark preset...');
 ...
 if (txt.includes('Columns:') || txt.includes('Columns: Default') || txt.includes('Columns: Mark')) {
 const r = el.getBoundingClientRect();
 ...
 // positioned near the Columns button (right side of page, x > 1000) that contains Mark
 const tbPos = await page.evaluate(() => {
 // Find all elements with exact text "Mark"
 const allTB = [];
 for (const el of document.querySelectorAll('*')) {
 if ((el.textContent || '').trim() === 'Mark') {
 const r = el.getBoundingClientRect();
 ...
 // If the dropdown is open but Mark is off-screen (y > viewport), scroll it into view
 ...
 if (!tbPos) { console.log(' ⚠ Mark option not found (no right-side match)'); return false; }
 ...
 if (!tbPos2) { console.log(' ⚠ Mark not in viewport after scroll'); return false; }
