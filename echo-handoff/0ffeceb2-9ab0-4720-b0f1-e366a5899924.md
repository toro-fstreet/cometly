...
## Cometly Access
- **Login:** toro6ot@gmail.com / 1getshitdone!
...
- **Column preset:** Toro-Bot (persists between sessions — custom columns are off-screen right, NOT reset)
...
## Column Mappings (Toro-Bot preset)
| col-id | Meaning |
|---|---|
| amount_spent | Spend |
...
```bash
cd /Users/toro/.openclaw/workspace
...
## Trigger Phrase
When Peter says **"Cometly report [X] days"** in #toro-peter, run the full pipeline immediately — no clarifying questions:
...
4. Reply in #toro-peter with the GitHub Pages link only
...
- **Prompt file:** `/Users/toro/.openclaw/workspace/cron-prompt-cometly.txt`
- **Output:** Posts 14-day report link to #toro-peter (C0AM1FYSA11)

## Output Rules
- All outputs → **#toro-peter** (C0AM1FYSA11) only. Never cross-post anywhere else unless explicitly directed by Peter in that session.
...
- **Contact ad attribution lookups:** Output to #toro-peter only. Never post to any other channel.
